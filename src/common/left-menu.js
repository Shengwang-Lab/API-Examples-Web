...
function __listToDom(list) {
  return list.map((item, index) => {
    ...
    <div class="sidebar-item" data-index="${index}">
      <a href="${item.url}" class="i18n" name="${item.title}">${item.defaultTitle}</a>
    </div>
    `
  }).join("")
}
...
function __initMenu() {
  ...
  ${__transListToMenuDom(SETUP_LIST, "settingMenu", "Setting")}
  ${__transListToMenuDom(BASIC_LIST, "quickMenu", "Quick Start")}
  ${__transListToMenuDom(ADVANCED_LIST, "advancedMenu", "Advanced Examples")}
  ${__transListToMenuDom(PLUGIN_LIST, "pluginMenu", "Plugin Examples")}
  ${__transListToMenuDom(othersList, "othersMenu", "Other Examples")}
  ${__transListToMenuDom(FRAMEWORK_LIST, "frameworkMenu", "Framework Examples")}
  </div >
    `
...
function __hightlightMenu() {
  ...
  const targetNode = $(`.sidebar-item:has(a[href = "${href}"])`)
